...
import { STAYCAY_ROLES, useStaycayRole } from '@/lib/mock';
...
export function DropdownMenuUser({ trigger }: { trigger: ReactNode }) {
  ...
  const { role, changeRole, label: roleLabel } = useStaycayRole();
      ...
      <DropdownMenuContent className="w-64" side="bottom" align="end">
        ...
        <DropdownMenuSub>
          ...
          <DropdownMenuSubContent className="w-56">
            <DropdownMenuRadioGroup
              value={role}
              onValueChange={(value) => changeRole(value as typeof role)}
            >
              {STAYCAY_ROLES.map((item) => (
                <DropdownMenuRadioItem
                  key={item.value}
                  value={item.value}
